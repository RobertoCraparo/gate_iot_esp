...
void handleFailure() {
    ...
    String failPayload = "{\"event\":\"auth\",\"status\":\"failed\",\"attempts\":" + String(failedAttempts) + "}";
    mqttClient.publish(topic_stato, failPayload.c_str());
    Serial.print("[MQTT TX] Topic: cancello/stato | Payload JSON: ");
    Serial.println(failPayload);

    if (failedAttempts >= 3) {
        isLocked = true;
        Serial.println("[SECURITY] SOGLIA SUPERATA. SISTEMA BLOCCATO.");

        String lockPayload = "{\"event\":\"alarm\",\"status\":\"system_locked\"}";
        mqttClient.publish(topic_stato, lockPayload.c_str());
        Serial.print("[MQTT TX] Topic: cancello/stato | Payload JSON: ");
